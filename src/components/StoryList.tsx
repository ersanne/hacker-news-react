...
import { Author, ExternalLink, Failure, Icon, Time } from './ui';

export default function StoryList({ items, selected, read, onRead, href, fallbackSource, onRetry }: {
  items: ItemResult[];
    ...
    {items.map(({ id, item, error: failed }, index) => {
      ...
      return <li key={id} data-story-id={id} className={`story-row ${selected === id ? 'selected' : ''} ${read.has(id) ? 'is-read' : ''}`}>
        ...
        <div className="story-content">
          <div className="story-domain">{url ? <ExternalLink href={url}>{domain(url)} <Icon name="arrow" size={11} /></ExternalLink> : <span>{fallbackSource}</span>}{read.has(id) && <span className="read-label">Read</span>}</div>
