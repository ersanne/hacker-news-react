...
export default function SearchPanel({ query, active, selected, read, onRead, saved, onToggleSaved, hideRead, onHideRead, hrefSuffix, refreshAt, onBusy }: {
  ...
  return <section className="feed-panel" hidden={!active} aria-label={`Search results for ${query}`}>
    ...
    <div className="feed-scroll" ref={scrolling} onScroll={event => { if (active) scrollTop.current = event.currentTarget.scrollTop; }}>
      ...
      <StoryList items={shown} selected={selected} read={read} onRead={onRead} saved={saved} onToggleSaved={onToggleSaved}
        href={id => `/item?id=${id}&q=${encodeURIComponent(query)}${hrefSuffix}`} fallbackSource="Hacker News" />
    ...
    <FeedFooter>Search by the Hacker News Algolia API<ExternalLink href="https://hn.algolia.com">hn.algolia.com <Icon name="arrow" size={12} /></ExternalLink></FeedFooter>
  </section>;
}
